...
function HomePage() {
    ...
    // Build query params for fetchItems
    ...
    useEffect(() => {
        dispatch(fetchItems(buildFetchParams()));
    }, [dispatch, buildFetchParams]);

    // Derived: sorted + price-filtered items (client-side, within current page)
    ...
    const handleReset = () => {
        setSearchText(""); setCategory(""); setSortBy("");
    ...
    // Cart helpers — clamp quantity to max allowed (min of stock and 10 - already in cart)
    const getMaxQty = (product: Item) => {
    ...
    const handleQuantityChange = (productId: string, value: string, product: Item) => {
        ...
        setQuantities(prev => ({ ...prev, [productId]: Math.min(Math.max(1, parsed), max) }));
    };
                        ...
                        <Box key={product._id}>
                            <Card sx={{ height: "100%", display: "flex", flexDirection: "column", boxShadow: 3 }}>
                                ...
                                <CardActions sx={{ flexDirection: "column", p: 2, gap: 1 }}>
                                    ...
                                    >
                                        {product.stock === 0 ? "Out of Stock" : maxQty <= 0 ? "מקסימום בעגלה" : "Add to Cart"}
                                    </Button>
                                    <Button
                                        variant="outlined"
                                        fullWidth
